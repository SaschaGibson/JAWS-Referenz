...
;JAWS script file for Office 2007 only
...
var
	;timestart and now for bench test / check for performance.
	;int TimeStart,
	;int now,
	handle hwnd, ;globalFocusWindow is unreliable
	handle hReal, ; globalRealWindow is unreliable
	int bSDM
;Let TimeStart = GetTickCount ()
let hwnd=GetFocus()
let hReal=GetRealWindow(hwnd)
Let bSDM = ( StringContains (GetWindowClass(hReal),wc_SDM)
|| StringContains (GetWindowClass (GetParent(hReal)) ,wc_SDM)
|| StringContains (GetWindowClass (GetParent(GetParent(hReal))),wc_SDM)
|| StringContains (GetWindowClass (GetParent(GetParent(hwnd))), wc_SDM)
|| StringContains (GetWindowClass (GetParent(GetParent(GetParent(hwnd)))), wc_SDM)
|| StringContains (GetWindowClass(hwnd),wc_SDM)
|| StringContains (GetWindowClass(GetParent(hwnd)),wc_bosa_sdmGeneral) )
;Let now = (GetTickCount ()-TimeStart)
return bSDM
EndFunction

;For the following two overrides, pass the common key through, rather than use JAWSfunction,
;to work around FSDom when not needing to use Enhanced Edit support.
;Ex. SpellCheck dialog, where rarely, the TabKey or ShiftTabKey function will fail
...
elIf GetWindowClass(hWindow) == wc_NetUIHWND then
	let obj=GetObjectFromEvent(hWindow, 0, 0, varChild)
	let sName = obj.accName(varChild)
	if StringContains (sName, scOffice2003) then
		;SayString (sName)
...
EndIf
if isSDMDlg() then
	let sHotkey=self::findHotkey(sPrompt)
...
If nLevel==-1 then
	...
	if inOptionsDialog(hwnd) then
		let sObjName=getObjectName(true,1) ;to check for control name in the parent of the object name
		if sObjName==sc_OptionsDlgCategories then
			sayMessage(ot_control_name,GetObjectName(true,1))
		endIf
		indicateControlType(GetObjectSubtypeCode(true,1),GetObjectName())
		return
	endIf
...
if nObjType == wt_ListBoxItem then
	...
	|| nLevel==2 then ; in list
		; for Microsoft Excel 2007 Data Sort dialog:
		if StringContains(stringLower(GetAppFileName()),sc_excel2007)
		&& sRealName==wn_Excel2007Sort then
			indicateControlType(nObjType,sObjName)
	...
	SayObjectTypeAndText()
EndIf
if !InOptionsDialog(hwnd)
&& GetObjectSubtypeCode()!=wt_menu then
	If ShouldItemSpeak (ot_access_key) == TUTOR_ALL  && nProductsList & product_JAWS then
		let iPunctLevel=GetJcfOption(opt_punctuation)
		SetJcfOption(opt_punctuation,0)
		let sHotkey=GetHotkey()
...
if InRibbons(hwndFocus) then
	...
	&& giEnterKey then
		...
		microsoft_excel_2007::SayFocusedWindow()
		return
	EndIf
...
&& getWindowClass (getParent (hwndFocus)) == wc_NetUIToolWindow)
	...
	Return
;no status bar toolbar in Outlook 2007:
...
if nCurMenuMode>0 then
	...
	if nType==wt_splitButton then
		...
		return
	endIf
	sayObjectTypeAndText() ; prevent extra call to FocusChangedEventEx causing double-speaking of split buttons in menus.
	return
EndIf
let sClass = GetWindowClass(hwndFocus)
...
&& GetObjectSubtypeCode(true)!=wt_buttonMenu then
	...
	return
EndIf
;prevent extra chatter when returning from message body window to subject field in Outlook 2007:
if gbOutlookIsActive
&& GetWindowClass(hwndPrevFocus)==wc_WwG then
	return microsoft_outlook_2007::SayFocusedWindow()
EndIf
...
&& inRibbons (getParent (getParent (focusWindow))))
	;sayObjectTypeAndText ()
...
&& (iWinType == WT_SDM || iWinType == WT_TABCONTROL) then
	let hFocus=GetFocus()
	if getWindowClass (hFocus)== cscListviewClass then
...
if nLevel < 2 then
	if InRibbons(hWnd) then
		...
		EndIf
		return
	elIf (getWindowClass (hwnd) == cwc_RichEdit20W
	&& inRibbons (getParent (getParent (hwnd))))
		;sayObjectTypeAndText ()
		;there are cases where SayObjectTypeAndText truncates, so get value from MSAA:
...
if GetObjectSubtypeCode(true)==wt_menu then
	SayObjectTypeAndText(0,includeContainerName)
	return
EndIf
if GetMenuMode()>0
...
if GetObjectSubtypeCode() != wt_menu then
	return false
EndIf
If GetObjectSubtypeCode(1,1) == wt_dialog_page then
	return true
EndIf
return false
EndFunction
...
&& getWindowStyleBits (hwnd) & ES_MULTILINE then
	return wt_MultiLine_edit
EndIf
;Fix problem where edit combos are improperly labeled edit rather than edit combo.
;These are off the ribbons:
if (getWindowSubtypeCode (hWnd) == WT_EDIT
...
elIf iSubtype==wt_splitButton then
	...
	elIf GetObjectSubtypeCode(true,1)==wt_listbox then
		return msgListbox
	endIf
endIf
...
if iObjectType==wt_menu then
	...
	&& iSpeakAccessKeys == TUTOR_ALL then
		...
		if StringContains(sHotKey,sc_AltComma) then
			let sHotKey=StringReplaceSubstrings(sHotkey,sc_AltComma,msgAltfollowedBy)
		EndIf
		SayUsingVoice(vctx_message,sHotKey,ot_access_key)
		SetJcfOption(opt_punctuation,iPunctLevel)
	EndIf
...
; To handle the split buttons in Alt+F menu in Office 2007...
let sFocusClassName = GetWindowClass (hwndFocus)
If nMenuMode > 0
&& sFocusClassName == wc_NetUIHWND
&& iObjectType == WT_SPLITBUTTON then
...
&& stringContains(getWindowClass(GetParent(hFocus)),cwc_dlg32770) then
	;if GetObjectSubtypeCode()==wt_listbox then
	if getWindowSubtypeCode (hFocus) == WT_LISTBOX then
		let sPrompt = GetObjectName(1)
		let oFocus = getObjectAtPoint(iRefData,getCursorCol(),getCursorRow()) ; note getFocusObject doesn't work as it gets the dlg rather than the child
		let sHotkey = oFocus.accKeyboardShortcut(iRefData)
		return sHotKey
...
&& (iType==wt_editCombo
|| iType==wt_edit) then
	let oFocus=getObjectFromEvent(hFocus,iObjID,iObjChildID,iRefData)
	if  oFocus then
		let sHotkey=oFocus.accKeyboardShortcut(iRefData)
		return sHotkey
	endIf
...
&& sScriptKeyName!=cksUpArrow then
	if nType==wt_edit
	&& stringCompare(GetWindowClass(hFocus),cwc_richEdit20W)==0
	&& !IsSdmDlg()  then
		let sHotKey=GetHotKey()
		if sHotKey==cscNull then
