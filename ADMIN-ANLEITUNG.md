...
```
Bei erfolgreicher Registrierung erscheint zusätzlich: `ScriptVersion: 2026-04-09-schtasks-sync`.
...
- Falls `schtasks.exe` nicht gefunden wird, das Skript in einer normalen Windows PowerShell/PowerShell-Session mit gültigem `%WINDIR%` starten.
- Testlauf erzwingen:
```powershell
