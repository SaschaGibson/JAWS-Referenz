...
include "Braille.jsh" ; headers globals and constants
include "Common.jsm"
...
if theName == "getslidewithfocus"
|| TheName == "handlecustomspellcheckwindows" then
	Return
endIf
return Unknown (TheName, IsScript, IsDueToRecursion)
endFunction

void Function initPptAppObj ()
; a combination of object and UIA calls causes focus and other issues:
var string WindowClass
if StringContains(GetWindowOwner(GetFocus()),sc_outllibr) then
	; must not load Powerpoint application pointer.
	;Let Outlook 2010 take over.
	return
endIf
WindowClass = GetWindowClass(GetFocus())
if WindowClass != wcPowerpointClient 
&& WindowClass != wcPaneClassDC 
&& WindowClass != wcPowerpointSlideShow then
	; do not allow application pointer to re-initialize.
	return
endIf
; OPpt is a global
;let oPpt = GetNativeOMFromMSAA() 
; 2016 and later need to prioritize getting pointer to the object directly,
; otherwise, if there is no shape with focus, we won't get an object.
; This becomes a problem when the user opens a presentation, and before doing anything else, presses f5.
; User needs to make sure to wait till JAWS says "Ready," like always,
; or the objects aren't initialized.
if ! OPPT then ; in case returning to slide show window from alt tab:
	let OPPT = GetNativeOMFromMSAA()
endIf
if !oPpt then ; try again.
	let oPpt = GetObject("powerpoint.application")
	if !oPpt then
		sayMessage(ot_JAWS_message,msgPleaseWait1)
		if WindowClass == wcPaneClassDC 
		|| WindowClass != wcPowerpointSlideShow then
			delay (10) ; don't build a message box over the slide show.
		else
			TimedMessageBox(GetAppMainWindow(GetFocus()),msgInitMessageBoxText,msgInitMessageBoxTitle,1000)
		endIf
		let oPpt = GetNativeOMFromMSAA()
		if !oPpt then
			let oPpt = GetObject("powerpoint.application")
	...
	EndIf
endIf
if oPPT.application then
	oPPT = oPPT.application
endIf
if oPPT then
	sayMessage(ot_JAWS_message,msgReady1)
;attach events:
	ComAttachEvents (oPPT, "PPT_")
endIf
EndFunction

void Function AutoStartEvent ()
PPTVersion = GetProgramVersion (GetAppFilePath ())
let gbMagicExists = getRunningFSProducts () & product_MAGic
let globalParaIndexPointer=1 ; for tracking which animated paragraph we are up to
let globalShapeIndex=1
let globalPriorSlideIndex=0
; note that the OPpt pointer is not initialized here as it causes initially
;displayed dialogs not to respond until the object is obtained.
;Instead we set the pointer when first necessary.
;see call to initPPTAppObj() in sayFocusedWindow() in microsoft powerpoint.jss)
;pause()
delay(1,true) ; prevent events firing before script set fully loads.
InitPPTAppObj()
let giReadingUserBufferChart=false
EndFunction

void Function AutoFinishEvent ()
; to keep the slide show readable when user alt tabs back to it:
if ! giSlideShowActive then
	ComRelease (oPpt, TRUE)
	let oPpt = null()
endIf
ComRelease (goFocusShape, TRUE)
let goFocusShape = null()
ComRelease (goFocusSlide, TRUE)
let goFocusSlide = null()
EndFunction

object function getTableFromShape (object shape)
; this function has a nesting level we usually don't go to,
; but makes sense here because we're looking at iterating nodes, and inside each iteration, iterating shapes.
; in reality, there's usually no more than one shape per node, if there is even one.
var object table, object nothing
if ! shape.hasTable then return nothing endIf
table = shape.table
if table != nothing then return table endIf
var object smartArt = shape.smartArt
var object images, object image
images = smartArt.allNodes
var object embeddedShapes, object embeddedShape
var int i, int count
forEach image in images
	if ! image.hidden then ; hidden may have the same problems as .visible property
		embeddedShapes = image.shapes
		count = embeddedShapes.count
		for i=1 to count
			embeddedShape = embeddedShapes(i)
			if embeddedShape.table then
				table = embeddedShape.table
				i = count+1 ; break at first found
			endIf
		endFor
	endIf
endForEach
...
; override from Office so as not to use UIA when other com calls are in progress:
if pptVersion < 16 then return GetUIAStatusBarText() endIf
var handle statusBar, handle parent = getParent (getFocus ())
if getWindowSubtypeCode (parent) != WT_MDICLIENT then return cscNull endIf
statusBar = getPriorWindow (parent)
if ! statusBar then return cscNull endIf
while (getFirstChild (statusBar))
	statusBar = getFirstChild (statusBar)
endWhile
return getListOfObjects (statusBar, cscBufferNewLine)
endFunction

int function FocusShapeUIAChildIsLink ()
; This is when a shape contains mixed content, text precedes a link.
; otherwise, the shape itself would return its type as hyperlink, or the JAWS subtype code would be hyperlink.
; Using UIA here is actually faster for the following reasons:
; 1. The FSUIA and treewalker global objects already exist within UIA.jss from where the functions are called.
; 2. This requires two calls and a property check.
; 3. If I did it through the object model, this would mean iterating all shapes for mixed content, analyzing the text range for each every time GetShapePlaceholderDescription and Braille callback functions are used.
; 4. Whenever you have a shape with mixed hyperlinks and text,
; the hyperlink will always be the first UIA child of the shape, because the text belongs to the shape's TextRange, not a separate FSUIAElement object.
var object element = FSUIAGetFocusedElement ()
if ! element then return FALSE endIf
element = FSUIAGetFirstChildOfElement (element)
if ! element then return FALSE endIf
return element.controlType == UIA_HyperlinkControlTypeID
endFunction

int function FocusedShapeContainsHyperlink ()
; don't use FSUIA while Com Calls from object models are in progress:
return getObjectSubtypeCode () == WT_LINK
; or focus shape contains a link as a child object,
; usually found when text precedes the link in the shape:
|| FocusShapeUIAChildIsLink ()
endFunction

int function getRelevantTypeForShape (object shape)
; This gets the first shape type from the SmartArt node,
; or the default shape.type as we used to.
; Calling this function to establish type should resolve a plethora of issues related to types that aren't showing properly.
var object nothing
var object smartArt = shape.smartArt
if smartArt == nothing then return shape.type endIf
var object images, object image
images = smartArt.allNodes
if images == nothing then return shape.type endIf
var object embeddedShapes, object embeddedShape
var int i, int count
forEach image in images
	if ! image.hidden then ; hidden may have the same problems as .visible property
		...
		for i=1 to count
			embeddedShape = embeddedShapes(i)
			; get type info from first object
			if embeddedShape.type then return embeddedShape.type endIf
		endFor
	endIf
endForEach
endFunction

int function OnThumbnailsView ()
return oPpt.activeWindow.activePane.viewType == ppViewThumbnails 
endFunction

void function UpdateFocusWhenEditable ()
; fixes a problem where when you press enter on a shape,
; it doesn't allow you to edit properly.
; merely calling these objects will set things right.
var
	string Text,
	object shape
...
var
	...
	object controls,
	object oTextRange
;let controls = oPpt.commandbars(CBFormatting).controls();doesn't exist in 2010, check the object browser
let oTextRange=oPpt.activeWindow.selection.textRange
Let nAlignment = oTextRange.ParagraphFormat.Alignment
if oTextRange.Font.Bold == vbTRUE then
	Let strBuffer = (strBuffer+msgBolded) + cscBufferNewLine
EndIf
if oTextRange.Font.Italic == vbTRUE then
	Let strBuffer = (strBuffer+msgItalicized) + cscBufferNewLine
EndIf
if oTextRange.Font.Underline == vbTRUE then
	Let strBuffer = (strBuffer+msgUnderlined) + cscBufferNewLine
EndIf
if nAlignment == ppAlignLeft then
	Let strBuffer = (strBuffer+msgAlignedLeft) + cscBufferNewLine
EndIf
if nAlignment == ppAlignCenter then
	Let strBuffer = (strBuffer+msgCentered) + cscBufferNewLine
EndIf
if nAlignment == ppAlignRight then
	Let strBuffer = (strBuffer+msgAlignedRight) + cscBufferNewLine
EndIf
if nAlignment == ppAlignJustify then
	Let strBuffer = (strBuffer+msgJustified) + cscBufferNewLine
EndIf
if GetCharacterAttributes () & attrib_highlight then
	Let strBuffer = (strBuffer+msgHighlighted)+cscBufferNewLine
EndIf
if oTextRange then
	Let strBuffer = (strBuffer+formatString(msgParagraphLevel1_L, intToString(oTextRange.indentLevel))+cscBufferNewLine)
	Let strBuffer = (strBuffer+formatString(cmsgColorInfo,GetColorName(oTextRange.font.color.rgb),GetcolorName(GetColorBackground()))+cscBufferNewLine)
	Let strBuffer = (strBuffer+GetAdditionalFontInfo (oTextRange))
EndIf
if isSameScript () then
	Let sFont = (FormatString (cMsgFontInformation, strBuffer))
	...
	let sFont = FormatString (cmsg38_L, strBuffer)
	SayFormattedMessage (ot_user_requested_information, sFont, strBuffer) ;"Font is"
endIf
EndFunction

Void Function ReportFont (string strButton)
var
	object textRange,
	int state
...
endIf
Let textRange = oPpt.activeWindow.selection.textRange
pause()
if stringCompare (strButton, btnBold) == 0 then
	let state = textRange.font.Bold
elif stringCompare (strButton, btnItalic) == 0 then
	Let state = textRange.font.Italic
elif stringCompare (strButton, btnUnderline) == 0 then
	Let state = textRange.font.Underline
else
	let state = oPpt.commandbars(CBFormatting).controls(strButton).state()
endIf
if state == -1 then
	SayFormattedMessage(ot_status,strButton+cscSpace+cmsg_on, cmsg_on)
else
	SayFormattedMessage(ot_status,strButton+cscSpace+cmsg_off, cmsg_off)
endIf
EndFunction

string function getCellContentsFromUIA ()
return GetCell ()
; or alternatively, have an internal function that will return the child text range to us?
endFunction

string Function getCellContents ()
;if FocusWindowSupportsUIA () then
	;return getCellContentsFromUIA ()
;endIf
return GetCell()
EndFunction
...
if sClass==wcUserControl then
	return PP_WT_USER_CONTROL+WT_CUSTOM_CONTROL_BASE
elif sClass==wcExcel7 then
	return PP_WT_XLWS+WT_CUSTOM_CONTROL_BASE
elif sClass!=wcPowerpointClient then
	if RibbonsActive ()
	|| isBackStageView(GetFocus ())
		; where edit spin boxes can erroneously be returning as edit instead of spinbox.
		if getWindowSubtypeCode (GetFocus ()) == WT_EDIT
		&& getObjectSubtypeCode () == WT_EDIT_SPINBOX then
	...
	endIf
endIf
let iSelType=OPpt.activeWindow.selection.type()
if iSelType==ppSelectionShapes then
	let oSel=OPpt.activeWindow.selection.shapeRange()
	if oSel.count<=1 then
		; check type
		let iShapeType=getRelevantTypeForShape (oSel(1))
		if oSel(1).hasTable then
		; where smart art has an embedded table:
			iShapeType = msoTable
		endIf
		if iShapeType==msoPlaceholder then
			return PP_WT_PLACEHOLDER+WT_CUSTOM_CONTROL_BASE
		elif iShapeType==msoTable then
			return PP_WT_TABLE+WT_CUSTOM_CONTROL_BASE
		else
			return PP_WT_SHAPE+WT_CUSTOM_CONTROL_BASE
		endIf
	elif oSel.count > 1 then
		return PP_WT_SHAPE_RANGE+WT_CUSTOM_CONTROL_BASE
	endIf
elif iSelType==ppSelectionSlides then
	let oSel=OPpt.activeWindow.selection.slideRange()
	if oSel.count<=1 then
		return PP_WT_SLIDE+WT_CUSTOM_CONTROL_BASE
	elif oSel.count > 1 then
		return PP_WT_SLIDE_RANGE+WT_CUSTOM_CONTROL_BASE
	endIf
elif iSelType==ppSelectionText then
	if getTableFromShape (oPpt.activeWindow.selection.shapeRange(1)) then
		return PP_WT_TABLE_CELL+WT_CUSTOM_CONTROL_BASE
	else
		return wt_edit
	endIf
Elif ! IsPPTEditMode ()
&& (oPpt.activeWindow.selection.slideRange(1)) then
	Return PP_WT_SLIDE + WT_CUSTOM_CONTROL_BASE
else
	return BrailleCallBackObjectIdentify()
endIf
...
int function BrailleAddObjectUserControlPrompt(int nType)
var
int CId,
string Name,
handle hFocus,
...
if IsSlideShowWithUIA () then return FALSE endIf ; No object calls interfering with UIA
;return (oPpt.activePresentation.slideShowWindow.view.state == ppSlideShowRunning)
EndFunction

int function IsSlideShowWithUIA ()
; Is the new UIA support for slide shows in progress?
if ! FocusWindowSupportsUIA () then return FALSE endIf
if GetObjectIsEditable () 
; hyperlinks show as isEditable (possibly because they can be clicked?) from in the slide show.
&& ! isVirtualPcCursor () then
	return FALSE
endIf
var string class = getWindowClass (getFocus ())
return isVirtualPcCursor () && (class == wcPowerpointSlideShow || class==wcPowerpointClient)
endFunction 
...
int function OnNotesPane ()
var
	int activePaneView
let activePaneView = oPpt.activeWindow.activePane.viewType
return activePaneView == ppViewNotesPage 
endFunction
...
string function getShapePlaceHolderDescriptionFromUIA ()
var string description, object element = FSUIAGetFocusedElement (), object childElement
if ! element return cscNull endIf
; when the shape type is unknown from the object model,
; and its child has a link,
; we actually want the image from the prior sibling as the UIA focus is technically wrong.
childElement = FSUIAGetFirstChildOfElement (element)
if childElement && childElement.controlType == UIA_HyperlinkControlTypeID then
	element = FSUIAGetPriorSiblingOfElement (element)
endIf
if ! element return cscNull endIf
description = element.localizedControlType
return description
endFunction
...
var
	string description,
	int shapeContainsLink = FocusedShapeContainsHyperlink (),
...
if PlaceHolderFormat == ppPlaceholderBitmap then
	...
	if shapeContainsLink then
		let description = msgHyperlink
	else
...
endIf
if stringIsBlank (description) then
	description = getShapePlaceHolderDescriptionFromUIA ()
endIf
return description
EndFunction
...
if not selectedShape.hasTable then
	...
	return
endIf
let table = getTableFromShape (selectedShape)
...
var
	int iType
let iType = getRelevantTypeForShape (shape)
if iType == MsoAutoShape then
  ...
  return GetOleObjDescription (shape)
	...
	if !giObjectOnly then
		; type may not be table but shape may contain one:
  ...
  return msg313_L ;"linked picture", ; 11,
	...
	var int mediaType = shape.MediaType
	if mediaType == ppMediaTypeMovie then
		return msgMediaTypeVideo
	elif MediaType == ppMediaTypeSound then
		return msgMediaTypeAudio
	else
...
ElIf iType == MsoHyperlinkShape then
	return msgHyperlink
elIf iType == msoSmartArt 
|| shape.smartArt then
		return msgSmartArtPlaceHolder
else
	return shape.name
endIf
EndFunction
...
void function processSmartArtImage (object image, int textOnly, int iRedirectToUserBuffer)
var
	int iOutputType,
	int i, int count,
	object textRange = image.TextFrame2.TextRange,
...
if iRedirectToUserBuffer then
	...
	iOutputType = ot_line 
endIf
var string alternativeText = image.alternativeText
If stringIsBlank (alternativeText) then alternativeText = image.name endIf
if ! stringIsBlank (alternativeText) then
	SayUsingVoiceOrBuffer (vctx_message, alternativeText, iOutputType, null())
endIf
if ! stringIsBlank (textRange.text) then
	SayUsingVoiceOrBuffer (vctx_message, textRange.text, iOutputType, null())
endIf
shapes = image.shapes
count = shapes.count
for i=1 to count
	shape = shapes(i)
	; use the function  from ppFuncCommon to avoid JAWS script lack of recursion.
	ppfuncCommon::SayShape (shape, textOnly, iRedirectToUserBuffer)
endFor
endFunction

void function SaySmartArtInfoForShape (object shape, int textOnly, int iRedirectToUserBuffer)
var
	int iOutputType,
	object smartArt = shape.smartArt,
	object images, object image,
	string tmp, string message
if ! smartArt 
|| shape.type != msoSmartArt then
	return
endIf
if iRedirectToUserBuffer then
	iOutputType = ot_user_buffer
...
endIf
images = smartArt.allNodes
if textOnly then ; using buffer
	SayUsingVoiceOrBuffer (VCTX_MESSAGE, msgSmartArtPlaceHolder, iOutputType, null)
endIf
forEach image in images
	if ! image.hidden then ; hidden may have the same problems as .visible property
		processSmartArtImage (image, textOnly, iRedirectToUserBuffer)
	endIf
endForEach
if textOnly then ; using buffer
	SayUsingVoiceOrBuffer (VCTX_MESSAGE, msgSmartArtPlaceHolderEnd, iOutputType, null)
endIf
endFunction

string function PrependNameIfNotDuplicate(string sInfo)
var
	string sName = GetObjectName ()
if !StringContainsCaseInsensitive (sInfo, sName)
&& !StringContainsCaseInsensitive (sName, sInfo)
	sInfo = StringConcatenate(sName, cscSpace, sInfo)
	endIf
return sInfo
EndFunction
...
var
	...
	object oAction,
	object s,
	int shapeType = getRelevantTypeForShape (shape)
if iRedirectToUserBuffer then
	let iOutputType = ot_user_buffer
else
	let iOutputType = ot_line ;ot_no_disable before 6.0
...
if iRedirectToUserBuffer && ! shape.visible  then return endIf
if ! TextOnly
|| shape.hasTable
|| shapeType == msoEmbeddedOleObject then
	sOutput = PrependNameIfNotDuplicate(GetShapeDescription (shape))
	SayUsingVoiceOrBuffer (VCTX_MESSAGE, sOutput, iOutputType, null)
	if oAction(ppMouseOver).action == ppActionHyperlink then
		sOutput = PrependNameIfNotDuplicate(formatString (msg79_L, getHyperlinkAddress (shape.actionSettings(ppMouseOver).hyperlink())))
		SayUsingVoiceOrBuffer (VCTX_MESSAGE, sOutput, iOutputType, null)
	elif oAction(ppMouseClick).action == ppActionHyperlink then
		sOutput = PrependNameIfNotDuplicate(formatString (msg80_L, getHyperlinkAddress (shape.actionSettings(ppMouseClick).hyperlink)))
		SayUsingVoiceOrBuffer (VCTX_MESSAGE, sOutput, iOutputType, null)
	endIf
endIf
if shape.HasTextFrame then
	...
	sOutput = PrependNameIfNotDuplicate(shape.alternativeText)
	SayUsingVoiceOrBuffer (vctx_message, sOutput, iOutputType, null)
elif shape.hasTable then
	var object table = getTableFromShape (shape)
	if iRedirectToUserBuffer then
	readTable (table, globalTableReadingMethod, vctx_message, iRedirectToUserBuffer)
	else
		sOutput = PrependNameIfNotDuplicate(getTableDimensions (table))
		SayUsingVoiceOrBuffer (vctx_message, sOutput, iOutputType, null)
	endIf
elif shapeType == msoGroup then
	forEach s in shape.groupItems
		if s.HasTextFrame &&  s.textFrame.hasText then
			...
			sOutput = PrependNameIfNotDuplicate(s.alternativeText)
			SayUsingVoiceOrBuffer (vctx_message, sOutput, iOutputType, null)
		EndIf
	endForEach
endIf
SaySmartArtInfoForShape (shape, textOnly, iRedirectToUserBuffer)
EndFunction
...
if type==ppSelectionShapes then
	...
	sayShape(goFocusShape,false, false)
...
Var
	...
	object oSlide,
	object oShape,
	object oSelection = oPpt.activeWindow.selection
if oSelection.HasChildShapeRange then
; contains smartArt
	let oShape = oSelection.ChildShapeRange(1)
	return oShape
endIf
...
if oSelection.HasChildShapeRange then
; contains smartArt
	let oFocus = oSelection.ChildShapeRange(1)
else
	let oFocus = oSelection.shapeRange(1)
endIf
;let oSlide = getSlideWithFocus ()
...
string Function GetCurrentSlideNotesAndCommentsInfo (optional int ignoreComments)
;Indicate there are one or more, e.g. 'Contains comments'
; set parameter ignoreComments to TRUE for places where only notes are required.
var
	object slide,
	int nComments,
	int nNotes,
...
; editing a slide, only notes are sought in a slide show.
...
while shapeIndex <= lastShapeIndex
	...
	if ! TextOnly
	|| shape.hasTable
	|| getRelevantTypeForShape (shape) == msoEmbeddedOleObject then
...
var
	object selection,
	int type
if (getWindowClass (getCurrentWindow ()) != wcPowerpointClient)
...
string function getSelectedTextFromObject ()
var object selection = oPpt.activeWindow.selection
if ! selection then return null endIf
...
if type == ppSelectionSlides then
	...
	while index <= count
		...
		; thumbnails view now always announces position correctly.
...
if focusClass == wcPowerpointClient then
	...
	&& iPaneType != ppViewNotesPage then
		if isCaretInTable () then
			let sText = formatString (msg26_L, getCellCoordinateString (), getTableDimensions (getTableFromShape (oPpt.activeWindow.selection.shapeRange(1))))
		else
...
;	**
	if iPaneType == ppViewSlide then
		UserBufferAddText (FormatString (msgScreenSensitiveHelpSlideAreaWithCount, sSlideNumber, oPpt.activePresentation.slides.count))
		userBufferAddText (cscBufferNewLine+msgScreenSensitiveHelpPowerPointSlideShowUIA)
	EndIf
...
if shape.HasTextFrame() then
	...
	readTable (getTableFromShape (shape), globalTableReadingMethod, vctx_message, iOutputType)
elif shapeType == msoGroup then
	foreach s in shape.groupItems
	if s.HasTextFrame() && s.textFrame.hasText then
		readParagraphs (s, textOnly, iRedirectToUserBuffer)
...
void function AnnounceEditingSlide ()
indicateControlType (wt_edit)
endFunction
...
if builtin::getCellCoordinates (col, row) && col && row then
...
if focusedShape.hasTable then
	let table = getTableFromShape (focusedShape)
...
forEach Shape2 in oCollection
	if shape2.name != shape.name then
		let testShapeDesc = getShapeDescription (shape2)
		let testTopLeftX = shape2.left
		let testTopLeftY = shape2.top
		let testBottomRightX = testTopLeftX+shape2.width
		let testBottomRightY = testTopLeftY+shape2.height
...
void function readSpeakersNotesInUIASlideShow ()
var
	object window, object notesPane,
	int index,
	int iRead ; whether to read notes or return an error that no notes exist for this slide.
window = fsUIAGetElementFromHandle (getRealWindow (getFocus ()))
if ! window then
	sayMessage(ot_error,msgSpeakerNotesError)
	return
endIf
var object condition = FSUIACreateStringPropertyCondition (UIA_AutomationIdPropertyId, "SlideNotesPane")
notesPane = window.findFirst(TreeScope_Subtree,condition)
if ! notesPane then
	sayMessage(ot_error,msgNotesPageNeedsPresenterView)
	return
endIf
var object notesTextElement = FSUIAGetFirstChildOfElement (notesPane)
if ! notesTextElement then
	sayMessage(ot_error,msgNotesPageNeedsPresenterView)
	return
endIf
; the following function is called passing an optional element, which won't actually get the focused element's text,
; but use the supplied notesTextElement.
var string notesText = FSUIAGetFocusedDocumentTextRangeText (notesTextElement)
if stringIsBlank (notesText) then
	sayMessage(ot_error,msgSpeakerNotesError)
	return
endIf
UserBufferClear ()
UserBufferActivate ()
userBufferAddText (notesText)
JAWSTopOfFile ()
sayAll ()
EndFunction
...
endFor
if userBufferIsActive () then
	var string bufferText = UserBufferGetText ()
	iRead = (! stringIsBlank (bufferText)
	&& stringLength (bufferText) > 1)
endIf
if iRead then
...
var
	string sDesc,
	object oShape
let oShape = goFocusShape
if ! oShape then
	updateNavigationGlobals()
	let oShape = goFocusShape
endIf
let sDesc = GetShapeDescription (oShape)
giFocusChangeTriggeredByDOMCall = true
;The following calls to oShape.actionSettings(ppMouseOver) and oShape.actionSettings(ppMouseClick) result in additional focus changed events.
;These focus changed events in turn cause a braille update which kicks off the cycle from the below code again resulting in an infinite loop.
;giFocusChangeTriggeredByDOMCall is set to true above and is used in FocusChangedEventEx to return early after being set back to false.
...
elIf oShape.alternativeText then
	sText = oShape.alternativeText
elIf oShape.name then
	sText = oShape.name
endIf
...
; Note the mode parameter will select the cell whose coordinates are being retrieved which may unintentionally change the caret location!
;Normally this should not happen because the internal code will usually succeed and thus not reach the object model code.
if getCellCoordinates (iCol, iRow, 1) && iCol && iRow then
	sCoordinates = formatString (msgTableCoordinates2, intToString (iCol), intToString (iRow))
...
var
	string header,
	string sContent
if giBrlTblHeader &HEADER_COLUMN then
	header = getColumnHeader ()
	if ! stringIsBlank (header) then
		BrailleAddString (header, 0,0,0)
	endIf
endIf
if giBrlTblHeader &HEADER_ROW then
	header = getRowHeader ()
	if ! stringIsBlank (header) then
		BrailleAddString (header, 0,0,0)
	endIf
endIf
; Try adding focused cell. If this succeeds, cursor tracking will be supported.
if BrailleAddFocusCell() then
	return true
endIf
; If it fails, just fall back to manually adding cell content.
...
var
	string sDim
let sDim = GetTableDimensions (getTableFromShape (oPpt.activeWindow.selection.shapeRange(1)))
...
string function getSmartArtImageLongDescription (object image)
var
	int i, int count,
	object textRange = image.TextFrame2.TextRange,
	object shape, object shapes,
	string result, string tmp,
	string ShapeText, string textRangeText = textRange.text,
	string alternativeText = image.alternativeText
If stringIsBlank (alternativeText) then alternativeText = image.name endIf
if ! stringIsBlank (alternativeText) then
	alternativeText = alternativeText+cscSpace
endIf
if ! stringIsBlank (textRangeText) then
	textRangeText = textRangeText+cscSpace
endIf
shapes = image.shapes
count = shapes.count
for i=1 to count
	shape = shapes(i)
	; use the function  from ppFuncCommon to avoid JAWS script lack of recursion.
	var string textFromShape = shape.TextFrame.TextRange.Text
	if ! stringIsBlank (textFromShape) then
		textFromShape = cscColon+cscSpace+textFromShape
	endIf
	tmp = ppFuncCommon::GetShapeDescription (shape)
	if ! stringIsBlank (tmp) then
		if ! stringIsBlank (textFromShape) then
			tmp = tmp+textFromShape
		endIf
	tmp = tmp+cscSpace
	endIf
	tmp = tmp+textFromShape
	shapeText = shapeText+tmp
endFor
...
string function getBrlSmartArtInfoForShape (object shape)
var
	object smartArt = shape.smartArt,
	object images, object image,
	string tmp, string message
if ! smartArt 
&& getRelevantTypeForShape (shape) != msoSmartArt then
	return cscNull
endIf
images = smartArt.allNodes
message = msgSmartArtPlaceholderBrl+cscSpace
forEach image in images
if	 ! image.hidden then ; hidden may have the same problems as visible property.
	; getSmartArtImageLongDescription will pad the necessary newline characters.
		message = message+getSmartArtImageLongDescription (image) 
	endIf
endForEach
message = stringTrimTrailingBlanks (message) ; remove any trailing spaces
return message
endFunction

int function BrailleAddObjectPHDesc (int nType)
var
	;object oShape = oPpt.activeWindow.selection.shapeRange(1),
	object oShape = goFocusShape,
	string smartArtInfo = getBrlSmartArtInfoForShape (oShape),
	string sPHDesc
let sPhDesc = GetShapePlaceHolderDescription (oShape)
if stringIsBlank (sPhDesc) then
	let sPhDesc = GetAutoshapeDescription (oShape)
endIf
if stringIsBlank (sPhDesc) then
	sPhDesc = oShape.alternativeText
endIf
if sPHDesc != cscNull then
	BrailleAddstring (sPHDesc, 0, 0, 0)
	brailleAddString (smartArtInfo, 0,0,0) ; embedded smart art graphics.
	return true
elIf oShape.type == msoSmartArt then ; Don't use getRelevantTypeForShape here because we want the placeholder type
; placeholder itself is nothing but Smart Art graphics.
	BrailleAddString (msgSmartArtPlaceholderBrl, 0,0,0)
	brailleAddString (smartArtInfo, 0,0,0)
	return true
else
	BrailleAddString (msg86_L, 0, 0, 0)
	brailleAddString (smartArtInfo, 0,0,0) ; embedded smart art graphics
...
String Function GetInchesFromPoints (int points)
Var
	string sInches,
	string sRounded
; Since the scripting language does not provide floating point caculations, we convert using the following kludge:
If points < 72 then ; less than one inch
	Let points = points*100/72 ; 72 points=1 inch.
	let sInches = IntToString(points)
	If StringToInt (SubString (sInches, 3, 1)) >= 6 then ; round up
		let points = (points+10)   ; round up
	EndIf
	let sInches = IntToString (points)
	let sRounded = cscPeriod + SubString (sInches, 1, 2)
Else
	Let points = points*100/72 ; 72 points=1 inch.
	let sInches = IntToString (points)
	if StringToInt (SubString (sInches, 4,1)) >= 6 then ; round up
		let points = (points+10)/10 ; Truncate to three digits.
	Else
		let points = points/10 ; Truncate to three digits.
	EndIf
	let sInches = IntToString (points)
	let sRounded = SubString (sInches, 1, 1) + cscPeriod + SubString (sInches, 2, 2)
EndIf
return sRounded
EndFunction
...
EndIf
if isPPTEditMode () then
	return BrailleAddObjectShapeText (wt_edit)
EndIf
return BrailleAddObjectName (nType)
EndFunction

int function BrailleAddObjectValue (int type)
if type == WT_EDIT && isPPTEditMode () && stringIsBlank (getLine ()) then
; some edits where selection is the only way to retrive the line:
	var string selection = getSelectedTextFromObject ()
	if ! stringIsBlank (selection) then
		BrailleAddString (selection, 0,0,ATTRIB_HIGHLIGHT)
		return TRUE
	endIf
endIf
...
; default table support
	return default::TableErrorEncountered (NavType)
endIf
var
	int bPost
